...
(function() {
    ...
    setTimeout(function() {
        ...
        if (!materialTypeId) {
            ...
            if (!materialTypeId && typeof window.materialSelection !== 'undefined' && window
                .materialSelection.material_type_id) {
        ...
        function loadThicknesses(edgeProfileId, materialTypeId) {
            console.log('loadThicknesses called with:', {
                edgeProfileId,
                materialTypeId
            });
            ...
            const url =
                `{{ route('calculator.edge.thicknesses') }}?edge_profile_id=${edgeProfileId}&material_type_id=${materialTypeId}`;
            ...
            fetch(url, {
                ...
                .then(data => {
                    ...
                    if (data.thicknesses && data.thicknesses.length > 0) {
                        ...
                        thicknessSelect.innerHTML =
                            '<option value="">No thicknesses available for this combination</option>';
                    }
                ...
                .catch(error => {
                    ...
                    thicknessSelect.innerHTML =
                        '<option value="">Error loading thicknesses. Please try again.</option>';
                    thicknessSelect.disabled = false;
        ...
        document.querySelectorAll('.edge-circle').forEach(circle => {
            circle.addEventListener('click', function() {
                ...
                const selectedEdges = (window.edgeFinishing && window.edgeFinishing
                        .selected_edges) ||
                ...
                if (this.classList.contains('selected')) {
                    ...
                    if (window.edgeFinishing && window.edgeFinishing.selected_edges) {
                        window.edgeFinishing.selected_edges = window.edgeFinishing
                            .selected_edges.filter(e => e !== edge);
                    }
                    if (edgeFinishing && edgeFinishing.selected_edges) {
                        edgeFinishing.selected_edges = edgeFinishing.selected_edges.filter(
                            e => e !== edge);
                    }
                } else {
                    this.classList.add('selected');
                    if (window.edgeFinishing) {
                        if (!window.edgeFinishing.selected_edges) window.edgeFinishing
                            .selected_edges = [];
