...
<div class="materials">
    ...
    <div class="d-flex align-items-center justify-content-center">
        <ul class="border-0 nav nav-tabs justify-content-center mb-5" id="materialsTab" role="tablist">
            ...
            <li class="nav-item" role="presentation">
                ...
                @endphp
                <button class="nav-link @if($key === 0) active @endif" id="{{ $slug }}-tab" data-bs-toggle="tab"
    ...
    <div class="tab-content" id="materialsTabContent">
        ...
        @endphp
        <div class="tab-pane fade @if($loop->first) show active @endif" id="{{ $slug }}" role="tabpanel"
            ...
            <div class="row">
                ...
                <div class="col-md-4 mb-4">
                    <div class="p-0 card border-0 rounded-0 position-relative product-col cutout-card {{ $selectedCutoutId == $item->id ? 'selected' : '' }}"
                        ...
                        <img src="{{ asset('uploads/cut-outs/' . $item->images->first()->image) }}" class="card-img-top"
                            alt="{{ $item->name }}" />
                        @else
                        <img src="{{ asset('uploads/cut-outs/default.jpg') }}" class="card-img-top"
                            alt="{{ $item->name }}" />
                        @endif
                        <div class="p-0 card-body text-center">
                            <div class="titleoverlay">
                                ...
                                <a href="#" data-bs-toggle="modal" data-bs-target="#cutoutModal-{{ $item->id }}"
                            ...
                            <div class="modal-body">
                                <div class="row">
                                    ...
                                    <div class="col-md-7 text-center">
                                        @if($item->images->first())
                                        <img src="{{ asset('uploads/cut-outs/' . $item->images->first()->image) }}"
                                            class="product-main-img" alt="{{ $item->name }}" />
                                        @else
                                        <img src="{{ asset('uploads/cut-outs/default.jpg') }}" class="product-main-img"
                                            alt="{{ $item->name }}" />
                                        @endif
                                    </div>
