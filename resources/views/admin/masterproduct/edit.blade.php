...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h3 class="mb-0 fw-bold">Edit Master Product</h3>
    ...
    <div class="card shadow-sm">
        <div class="card-body">
            ...
            <form action="{{ route('admin.masterproduct.update', $product->id) }}" method="POST">
                @csrf
                <div class="row">
                    @php
                        $selectedMaterialCategoryId = old('material_category_id', $selectedCategories['material'] ?? optional($product->material)->material_category_id);
                        $selectedTypeCategoryId = old('material_type_category_id', $selectedCategories['material_type'] ?? optional($product->materialType)->material_type_category_id);
                        $selectedLayoutCategoryId = old('material_layout_category_id', $selectedCategories['layout'] ?? optional($product->materialLayout)->material_layout_category_id);
                        $selectedSinkCategoryId = old('sink_category_id', $selectedCategories['sink'] ?? optional($product->sink)->sink_categorie_id);
                        $selectedCutOutCategoryId = old('cut_outs_category_id', $selectedCategories['cut_outs'] ?? optional($product->cutOut)->cut_outs_category_id);
                    @endphp
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select id="material_category" name="material_category_id" class="form-control">
                            ...
                            @foreach($materialCategories as $cat)
                                <option value="{{ $cat->id }}" {{ (string) $selectedMaterialCategoryId === (string) $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="material_id" id="material_id" class="form-control" data-selected="{{ old('material_id', $product->material_id) }}">
                            <option value="">Select Material</option>
                            @foreach($materials as $material)
                                <option value="{{ $material->id }}" {{ (string) old('material_id', $product->material_id) === (string) $material->id ? 'selected' : '' }}>{{ $material->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select id="type_category" name="material_type_category_id" class="form-control">
                            ...
                            @foreach($typeCategories as $cat)
                                <option value="{{ $cat->id }}" {{ (string) $selectedTypeCategoryId === (string) $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="material_type_id" id="material_type_id" class="form-control" data-selected="{{ old('material_type_id', $product->material_type_id) }}">
                            <option value="">Select Material Type</option>
                            @foreach($materialTypes as $type)
                                <option value="{{ $type->id }}" {{ (string) old('material_type_id', $product->material_type_id) === (string) $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select id="layout_category" name="material_layout_category_id" class="form-control">
                            ...
                            @foreach($layoutCategories as $cat)
                                <option value="{{ $cat->id }}" {{ (string) $selectedLayoutCategoryId === (string) $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="material_layout_id" id="material_layout_id" class="form-control" data-selected="{{ old('material_layout_id', $product->material_layout_id) }}">
                            <option value="">Select Layout</option>
                            @foreach($materialLayouts as $layout)
                                <option value="{{ $layout->id }}" {{ (string) old('material_layout_id', $product->material_layout_id) === (string) $layout->id ? 'selected' : '' }}>{{ $layout->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="material_edge_id" id="material_edge_id" class="form-control">
                            ...
                            @foreach($edges as $edge)
                                <option value="{{ $edge->id }}" {{ (string) old('material_edge_id', $product->material_edge_id) === (string) $edge->id ? 'selected' : '' }}>{{ $edge->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="back_wall_id" id="back_wall_id" class="form-control">
                            ...
                            @foreach($backWalls as $backWall)
                                <option value="{{ $backWall->id }}" {{ (string) old('back_wall_id', $product->back_wall_id) === (string) $backWall->id ? 'selected' : '' }}>{{ $backWall->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select id="sink_category_id" name="sink_category_id" class="form-control">
                            ...
                            @foreach($sinkCategories as $sinkCategory)
                                <option value="{{ $sinkCategory->id }}" {{ (string) $selectedSinkCategoryId === (string) $sinkCategory->id ? 'selected' : '' }}>{{ $sinkCategory->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="sink_id" id="sink_id" class="form-control" data-selected="{{ old('sink_id', $product->sink_id) }}">
                            <option value="">Select Sink</option>
                            @foreach($sinks as $sink)
                                <option value="{{ $sink->id }}" {{ (string) old('sink_id', $product->sink_id) === (string) $sink->id ? 'selected' : '' }}>{{ $sink->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select id="cut_outs_category_id" name="cut_outs_category_id" class="form-control">
                            ...
                            @foreach($cutOutsCategories as $cutOutsCategory)
                                <option value="{{ $cutOutsCategory->id }}" {{ (string) $selectedCutOutCategoryId === (string) $cutOutsCategory->id ? 'selected' : '' }}>{{ $cutOutsCategory->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="cut_outs_id" id="cut_outs_id" class="form-control" data-selected="{{ old('cut_outs_id', $product->cut_outs_id) }}">
                            <option value="">Select Cut Out</option>
                            @foreach($cutOuts as $cutOut)
                                <option value="{{ $cutOut->id }}" {{ (string) old('cut_outs_id', $product->cut_outs_id) === (string) $cutOut->id ? 'selected' : '' }}>{{ $cutOut->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="color_id" id="color_id" class="form-control">
                            ...
                            @foreach($colors as $color)
                                <option value="{{ $color->id }}" {{ (string) old('color_id', $product->color_id) === (string) $color->id ? 'selected' : '' }}>{{ $color->name }}</option>
                            @endforeach
                        </select>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    dependencies.forEach(({ trigger, target }) => {
        if (trigger && trigger.value) {
            const selectedValue = target ? target.getAttribute('data-selected') : '';
            if (target && selectedValue) {
                target.dataset.selected = selectedValue;
            }
